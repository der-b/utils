...
int check_regex()
{
	...
	buf[1] = 0;
	ret =
	    regex_helper(URLPHARSER_LOWALPHA "*", LOWALPHA,
			 REPORT_ON_FAILURE);
	if (UT_SUCCESS != ret) {
		return ret;
	}
	...
	for (buf[0] = 'A'; buf[0] <= 'Z'; buf[0]++) {
		ret =
		    regex_helper(URLPHARSER_LOWALPHA, buf,
				 REPORT_ON_SUCCESS);
		if (UT_SUCCESS == ret) {
			return UT_FAIL;
		}
	...
	ret =
	    regex_helper(URLPHARSER_UPALPHA "*", UPALPHA,
			 REPORT_ON_FAILURE);
	if (UT_SUCCESS != ret) {
		return ret;
	}
	...
	for (buf[0] = 'a'; buf[0] <= 'z'; buf[0]++) {
		ret =
		    regex_helper(URLPHARSER_UPALPHA, buf,
				 REPORT_ON_SUCCESS);
		if (UT_SUCCESS == ret) {
			return UT_FAIL;
		}
	...
	ret =
	    regex_helper(URLPHARSER_ALPHA "*", UPALPHA LOWALPHA,
			 REPORT_ON_FAILURE);
	if (UT_SUCCESS != ret) {
		return ret;
	}
	...
	for (buf[0] = '0'; buf[0] <= '9'; buf[0]++) {
		ret =
		    regex_helper(URLPHARSER_ALPHA, buf, REPORT_ON_SUCCESS);
		if (UT_SUCCESS == ret) {
			return UT_FAIL;
		}
	}
	...
	for (buf[0] = 'a'; buf[0] <= 'z'; buf[0]++) {
		ret =
		    regex_helper(URLPHARSER_DIGIT, buf, REPORT_ON_SUCCESS);
		if (UT_SUCCESS == ret) {
			return UT_FAIL;
		}
	}
	...
	ret =
	    regex_helper(URLPHARSER_ALPHANUM "*", LOWALPHA UPALPHA NUM,
			 REPORT_ON_FAILURE);
	if (UT_SUCCESS != ret) {
		return ret;
	}
	...
	for (i = 0; i < strlen(MARK_RESERVED); i++) {
		buf[0] = MARK_RESERVED[i];
		ret =
		    regex_helper(URLPHARSER_ALPHANUM, buf,
				 REPORT_ON_SUCCESS);
		if (UT_SUCCESS == ret) {
			return UT_FAIL;
		}
	...
	for (i = 0; i < strlen(RESERVED); i++) {
		buf[0] = RESERVED[i];
		ret =
		    regex_helper(URLPHARSER_MARK, buf, REPORT_ON_SUCCESS);
		if (UT_SUCCESS == ret) {
			return UT_FAIL;
		}
	}
	...
	ret =
	    regex_helper(URLPHARSER_RESERVED "*", RESERVED,
			 REPORT_ON_FAILURE);
	if (UT_SUCCESS != ret) {
		return ret;
	}
	...
	for (i = 0; i < strlen(MARK); i++) {
		buf[0] = MARK[i];
		ret =
		    regex_helper(URLPHARSER_RESERVED, buf,
				 REPORT_ON_SUCCESS);
		if (UT_SUCCESS == ret) {
			return UT_FAIL;
		}
	...
	ret =
	    regex_helper(URLPHARSER_HEX "*", NUM "abcdefABCDEF",
			 REPORT_ON_FAILURE);
	if (UT_SUCCESS != ret) {
		return ret;
	}
	...
	ret =
	    regex_helper(URLPHARSER_SCHEME, "ht-t.p+s", REPORT_ON_FAILURE);
	if (UT_SUCCESS != ret) {
		return ret;
	}

	ret =
	    regex_helper(URLPHARSER_SCHEME, "h012t-t.p+s",
			 REPORT_ON_FAILURE);
	if (UT_SUCCESS != ret) {
		return ret;
	}
	...
	for (i = 0; i < strlen(MARK_RESERVED); i++) {
		buf[0] = MARK_RESERVED[i];
		ret =
		    regex_helper(URLPHARSER_SCHEME, buf,
				 REPORT_ON_SUCCESS);
		if (UT_SUCCESS == ret) {
			return UT_FAIL;
		}
	...
	ret =
	    regex_helper(URLPHARSER_SEGMENT,
	...
	ret =
	    regex_helper(URLPHARSER_PATH_SEGMENTS, "This/is/a/Path",
			 REPORT_ON_FAILURE);
	if (UT_SUCCESS != ret) {
		return ret;
	}
	...
	ret =
	    regex_helper(URLPHARSER_ABSOLUTEURI, "http://der-b.com",
			 REPORT_ON_FAILURE);
	if (UT_SUCCESS != ret) {
		return ret;
	}
	...
	ret =
	    regex_helper(URLPHARSER_ABSOLUTEURI,
	...
	ret =
	    regex_helper(URLPHARSER_FRAGMENT,
			 NUM LOWALPHA "%%4a" UPALPHA MARK RESERVED,
			 REPORT_ON_FAILURE);
	if (UT_SUCCESS != ret) {
		return ret;
	...
	ret =
	    regex_helper(URLPHARSER_REL_SEGMENT,
			 NUM LOWALPHA "%%4a" UPALPHA MARK,
			 REPORT_ON_FAILURE);
	if (UT_SUCCESS != ret) {
		return ret;
	...
	ret =
	    regex_helper(URLPHARSER_REL_PATH, "../test/bla.html",
			 REPORT_ON_FAILURE);
	if (UT_SUCCESS != ret) {
		return ret;
	}
	...
	ret =
	    regex_helper(URLPHARSER_REL_PATH, "/test.html",
			 REPORT_ON_SUCCESS);
	if (UT_SUCCESS == ret) {
		return UT_FAIL;
	}
	...
	ret =
	    regex_helper(URLPHARSER_RELATIVEURI, "../test/bla.html",
			 REPORT_ON_FAILURE);
	if (UT_SUCCESS != ret) {
		return ret;
	}
	...
	ret =
	    regex_helper(URLPHARSER_RELATIVEURI, "//test/bla.html?abc=def",
			 REPORT_ON_FAILURE);
	if (UT_SUCCESS != ret) {
		return ret;
	}
	...
	ret =
	    regex_helper(URLPHARSER_RELATIVEURI,
	...
	ret =
	    regex_helper(URLPHARSER_URI_REFERENCE, "../test/bla.html",
			 REPORT_ON_FAILURE);
	if (UT_SUCCESS != ret) {
		return ret;
	}
	...
	ret =
	    regex_helper(URLPHARSER_URI_REFERENCE,
	...
	ret =
	    regex_helper(URLPHARSER_URI_REFERENCE, "http://der-b.com",
			 REPORT_ON_FAILURE);
	if (UT_SUCCESS != ret) {
		return ret;
	}
