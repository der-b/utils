		  ...
		  char *base64, size_t len_base64)
{
	...
	for (tmp = rest = i = j = 0; i < len_base64 && j < len_bytes; ++i) {
		if (6 == rest) {
			base64[i] = base64_toChar(0x3f & tmp);
			rest = 0;
			continue;
		}
		base64[i] =
...
int base64_decode(const char *base64, uint8_t * bytes, size_t * len_bytes)
{
	...
	for (restB = rest64 = j = i = 0; i < *len_bytes && j < len_b64;
	     ++i, ++j) {
		if (!rest64) {
			tmp = base64_toBin(base64[j]);
			if (0 > tmp) {
