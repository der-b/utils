...
LRet _list_array_realloc(List * list)
{
    ...
    if (_LIST_ARRAY_MINSIZE > larray->max_count) {
        ...
        if (__MUL(len, _LIST_ARRAY_MINSIZE, L_DATASIZE(list))) {
            goto err;
        }
    ...
    // increase size
        ...
        if (   __MUL(len,  larray->max_count, 2)
            || __MUL(len2, len,               L_DATASIZE(list))) {
            goto err;
        }
    ...
    // decrese size but not smaler then _LIST_ARRAY_MINSIZE
        ...
        if (larray->max_count / 2 <= _LIST_ARRAY_MINSIZE) {
            ...
            if (__MUL(len, larray->max_count / 2, L_DATASIZE(list))) {
                goto err;
            }
 ...
 * list_array_size
 */
LIndex list_array_size(List * list)
{
    struct list_array *larray;
...
void *list_array_pushback(List * list, void *data)
{
    ...
    if (__MUL(offset, larray->count, L_DATASIZE(list))) {
        goto err;
    }
...
LRet list_array_popback(List * list, void *data)
{
    ...
    if (data) {
        ...
        memcpy(data, (((uint8_t *) larray->data) + offset), L_DATASIZE(list));
    }
...
void *list_array_last(List * list)
{
    ...
    if (__MUL(offset, (larray->count - 1), L_DATASIZE(list))) {
        goto err;
    }
...
void *list_array_get(List * list, LIndex pos)
{
    ...
    if (__MUL(offset, pos, L_DATASIZE(list))) {
        goto err;
    }
...
void *list_array_insert(List * list, LIndex pos, void *data)
{
    ...
    for (dptr = ((uint8_t *) larray->data) + offset;
         dptr > stop; 
         dptr -= L_DATASIZE(list)) {

        memcpy(dptr, dptr - L_DATASIZE(list), L_DATASIZE(list));
    }

    if (data) {
        memcpy(dptr, data, L_DATASIZE(list));
    } else {
        memset(dptr, 0, L_DATASIZE(list));
    }
...
LRet list_array_remove(List * list, LIndex pos, void *data)
{
    ...
    if (__MUL(offset_end, larray->count, L_DATASIZE(list))) {
        goto err_inc;
    }
...
LRet list_array_removelast(List * list, void *del, void *data)
{
    ...
    if (data) {
        memcpy(data, dptr, L_DATASIZE(list));
    }

    for (; dptr < end; dptr += L_DATASIZE(list)) {
        memcpy(dptr, dptr + L_DATASIZE(list), L_DATASIZE(list));
    }
