...
/* 
 * if LOG_THREAD is defined, than the logging also prints the thread id
 */
// #define LOG_THREAD
...
#ifdef LOG_THREAD
        #include <pthread.h>
        #define LOG_THREAD_FMT "%lu:"
        #define LOG_THREAD_FUNC pthread_self(),
#else
        #define LOG_THREAD_FMT
        #define LOG_THREAD_FUNC
#endif
...
#define DEBUG(msg, ...)\
    (\
      fprintf(\
        LOG_FD,\
        LOG_THREAD_FMT "%s:%d:debug:   " msg "%s%s\n",\
        LOG_THREAD_FUNC \
        __FILE__,\
        __LINE__,\
        ##__VA_ARGS__,\
...
#define WARN(msg, ...)\
    (\
      fprintf(\
        LOG_FD,\
        LOG_THREAD_FMT "%s:%d:warn:   " msg "%s%s\n",\
        LOG_THREAD_FUNC \
        __FILE__,\
        __LINE__,\
        ##__VA_ARGS__,\
...
#define ERROR(msg, ...)\
    (\
      fprintf(\
        LOG_FD,\
        LOG_THREAD_FMT "%s:%d:error:   " msg "%s%s\n",\
        LOG_THREAD_FUNC \
        __FILE__,\
        __LINE__,\
	##__VA_ARGS__, \
...
#define CRIT(msg, ...)\
    (\
      fprintf(\
        LOG_FD,\
        LOG_THREAD_FMT "%s:%d:crit:   " msg "%s%s\n",\
        LOG_THREAD_FUNC \
        __FILE__,\
        __LINE__,\
        ##__VA_ARGS__,\
